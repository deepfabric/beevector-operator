...
import (
	...
	"github.com/infinivision/hyena-operator/pkg/controller/hyenacluster"
	"github.com/infinivision/hyena-operator/version"
...
func init() {
	...
	flag.BoolVar(&controller.ClusterScoped, "cluster-scoped", true, "Whether hyena-operator should manage kubernetes cluster wide Hyena Clusters")
...
func main() {
	...
	hcController := hyenacluster.NewController(kubeCli, cli, informerFactory, kubeInformerFactory)
	...
	onStarted := func(stopCh <-chan struct{}) {
		hcController.Run(workers, stopCh)
	}
	onStopped := func() {
		glog.Fatalf("leader election lost")
	}

	// leader election for multiple hyena-controller-manager
