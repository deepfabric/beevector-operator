...
import (
	...
	"github.com/infinivision/hyena-operator/pkg/pb/metapb"
	pdapi "github.com/infinivision/hyena-operator/pkg/pdapi"
...
func TestPodControlUpdateMetaInfoSuccess(t *testing.T) {
	...
	hc := newHyenaCluster()
	pod := newPod(cc)
	fakeClient, prophetControl, podLister, _, recorder := newFakeClientRecorderAndPDControl()
	control := NewRealPodControl(fakeClient, prophetControl, podLister, recorder)
	pdClient := NewFakePDClient()
	pdControl.SetProphetClient(hc, pdClient)
	pdClient.AddReaction(GetClusterActionType, func(action *Action) (interface{}, error) {
		return "222", nil
	})
	member1 := &PdMember{Name: "prophet1", MemberId: id1, PeerUrls: []string{"http://127.0.0.1:2381"}}
	member2 := &PdMember{Name: "prophet2", MemberId: id2, PeerUrls: []string{"http://127.0.0.1:2382"}}
	member3 := &PdMember{Name: "prophet3", MemberId: id3, PeerUrls: []string{"http://127.0.0.1:2383"}}
	...
	_, err := control.UpdateMetaInfo(hc, pod)
...
func TestPodControlUpdateMetaInfoGetMemberFailed(t *testing.T) {
	...
	pod.Labels[label.ComponentLabelKey] = label.ProphetLabelVal
	_, err := control.UpdateMetaInfo(cc, pod)
	g.Expect(err).To(HaveOccurred())
...
func TestPodControlUpdateMetaInfoConflictSuccess(t *testing.T) {
	...
	control := NewRealPodControl(fakeClient, pdControl, podLister, recorder)
	pdClient := NewFakeProphetClient()
	pdControl.SetPDClient(hc, pdClient)
	pdClient.AddReaction(GetClusterActionType, func(action *Action) (interface{}, error) {
	...
	g.Expect(updatePod.Labels[label.ClusterIDLabelKey]).To(Equal("hyena-cluster-demo"))
...
func TestPodControlUpdatePod(t *testing.T) {
	...
	fakeClient, pdControl, podLister, _, recorder := newFakeClientRecorderAndProphetControl()
	control := NewRealPodControl(fakeClient, pdControl, podLister, recorder)
	fakeClient.AddReactor("update", "pods", func(action core.Action) (bool, runtime.Object, error) {
		update := action.(core.UpdateAction)
		return true, update.GetObject(), nil
...
func newPod(cc *v1alpha1.HyenaCluster) *corev1.Pod {
	return &corev1.Pod{
