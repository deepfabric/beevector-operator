...
func TestStoreMemberManagerSyncCreate(t *testing.T) {
	...
	type testcase struct {
		name                     string
		prepare                  func(cluster *v1alpha1.HyenaCluster)
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		cc.Status.Prophet.Members = map[string]v1alpha1.PDMember{
			"prophet-0": {Name: "prophet-0", Health: true},
			"prophet-1": {Name: "prophet-1", Health: true},
			"prophet-2": {Name: "prophet-2", Health: true},
		}
		cc.Status.Prophet.StatefulSet = &apps.StatefulSetStatus{ReadyReplicas: 3}
...
func TestStoreMemberManagerSyncUpdate(t *testing.T) {
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		if test.errWhenGetStores {
			pdClient.AddReaction(controller.GetStoresActionType, func(action *controller.Action) (interface{}, error) {
				return nil, fmt.Errorf("failed to get stores from prophet cluster")
			})
		} else {
			pdClient.AddReaction(controller.GetStoresActionType, func(action *controller.Action) (interface{}, error) {
				return test.pdStores, nil
	...
	tests := []testcase{
		{
			...
			// prophetStores:            &controller.StoresInfo{Count: 0, Stores: []*pdapi.StoreInfo{}},
			// tombstoneStores:          &controller.StoresInfo{Count: 0, Stores: []*pdapi.StoreInfo{}},
...
*/
func TestStoreMemberManagerSyncHyenaClusterStatus(t *testing.T) {
	g := NewGomegaWithT(t)
	type testcase struct {
		name                      string
		updateCC                  func(*v1alpha1.HyenaCluster)
		upgradingFn               func(corelisters.PodLister, controller.PDControlInterface, *apps.StatefulSet, *v1alpha1.HyenaCluster) (bool, error)
		errWhenGetStores          bool
		storeInfo                 *controller.StoresInfo
		errWhenGetTombstoneStores bool
		tombstoneStoreInfo        *controller.StoresInfo
		errExpectFn               func(*GomegaWithT, error)
		ccExpectFn                func(*GomegaWithT, *v1alpha1.HyenaCluster)
	}
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		err := pdmm.syncHyenaClusterStatus(cc, set)
		if test.errExpectFn != nil {
			test.errExpectFn(g, err)
		}
	...
	tests := []testcase{
		{
			...
			updateCC: nil,
			upgradingFn: func(lister corelisters.PodLister, controlInterface controller.PDControlInterface, set *apps.StatefulSet, cluster *v1alpha1.HyenaCluster) (bool, error) {
			...
			ccExpectFn: func(g *GomegaWithT, cc *v1alpha1.HyenaCluster) {
		...
		},
		{
			...
			updateCC: func(cc *v1alpha1.HyenaCluster) {
		...
		{
			name: "LastHeartbeatTS is zero, HyenaClulster LastHeartbeatTS is not zero",
			updateCC: func(cc *v1alpha1.HyenaCluster) {
		...
		{
			name: "LastHeartbeatTS is zero, HyenaClulster LastHeartbeatTS is zero",
			updateCC: func(cc *v1alpha1.HyenaCluster) {
				cc.Status.Store.Stores = map[string]v1alpha1.KVStore{}
				cc.Status.Store.Stores["333"] = v1alpha1.KVStore{
		...
		{
			name: "LastHeartbeatTS is not zero, HyenaClulster LastHeartbeatTS is zero",
			updateCC: func(cc *v1alpha1.HyenaCluster) {
				cc.Status.Store.Stores = map[string]v1alpha1.KVStore{}
				cc.Status.Store.Stores["333"] = v1alpha1.KVStore{
					LastHeartbeatTime: metav1.Time{Time: time.Time{}},
...
func newFakeStoreMemberManager(hc *v1alpha1.HyenaCluster) (
	*storeMemberManager, *controller.FakeStatefulSetControl,
	*controller.FakeServiceControl, *controller.FakeProphetClient, cache.Indexer, cache.Indexer) {
	cli := fake.NewSimpleClientset()
	kubeCli := kubefake.NewSimpleClientset()
	prophetControl := controller.NewFakeProphetControl()
	prophetClient := controller.NewFakeProphetClient()
	prophetControl.SetProphetClient(hc, prophetClient)
	setInformer := kubeinformers.NewSharedInformerFactory(kubeCli, 0).Apps().V1beta1().StatefulSets()
	svcInformer := kubeinformers.NewSharedInformerFactory(kubeCli, 0).Core().V1().Services()
	tcInformer := informers.NewSharedInformerFactory(cli, 0).Deepfabric().V1alpha1().HyenaClusters()
	...
	return pxmm, setControl, svcControl, prophetClient, podInformer.Informer().GetIndexer(), nodeInformer.Informer().GetIndexer()
}
