...
func TestStoreScalerScaleIn(t *testing.T) {
	...
	type testcase struct {
		name           string
		storeUpgrading bool
		storeFun       func(cc *v1alpha1.HyenaCluster)
		delStoreErr    bool
		hasPVC         bool
		storeIDSynced  bool
		pvcUpdateErr   bool
...
func newFakeStoreScaler() (*storeScaler, *controller.FakeProphetControl, cache.Indexer, cache.Indexer, *controller.FakePVCControl) {
	...
	return &storeScaler{generalScaler{pdControl, pvcInformer.Lister(), pvcControl}, podInformer.Lister()},
		prophetControl, pvcInformer.Informer().GetIndexer(), podInformer.Informer().GetIndexer(), pvcControl
}

func normalStoreFun(cc *v1alpha1.HyenaCluster) {
	cc.Status.Store.Stores = map[string]v1alpha1.KVStore{
		"1": {
			ID:      "1",
...
func tombstoneStoreFun(cc *v1alpha1.HyenaCluster) {
	cc.Status.Store.TombstoneStores = map[string]v1alpha1.Store{
		"1": {
			ID:      "1",
			PodName: ordinalPodName(v1alpha1.StoreMemberType, cc.GetName(), 4),
