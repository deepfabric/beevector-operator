...
type Scaler interface {
	...
	ScaleOut(*v1alpha1.HyenaCluster, *apps.StatefulSet, *apps.StatefulSet) error
	// ScaleIn scales in the cluster
	ScaleIn(*v1alpha1.HyenaCluster, *apps.StatefulSet, *apps.StatefulSet) error
}
...
func (gs *generalScaler) deleteDeferDeletingPVC(hc *v1alpha1.HyenaCluster,
	...
	return skipReason, gs.pvcControl.DeletePVC(hc, pvc)
}
...
func ordinalPodName(memberType v1alpha1.MemberType, hcName string, ordinal int32) string {
	return fmt.Sprintf("%s-%s-%d", hcName, memberType, ordinal)
}
