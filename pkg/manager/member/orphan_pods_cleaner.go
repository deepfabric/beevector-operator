...
const (
	skipReasonOrphanPodsCleanerIsNotPDOrStore = "orphan pods cleaner: member type is not prophet or store"
...
type OrphanPodsCleaner interface {
	Clean(*v1alpha1.HyenaCluster) (map[string]string, error)
}
...
func (opc *orphanPodsCleaner) Clean(hc *v1alpha1.HyenaCluster) (map[string]string, error) {
	ns := hc.GetNamespace()
	// for unit test
	skipReason := map[string]string{}

	selector, err := label.New().Instance(hc.GetLabels()[label.InstanceLabelKey]).Selector()
	if err != nil {
	...
	for _, pod := range pods {
		...
		err = opc.podControl.DeletePod(hc, pod)
		if err != nil {
			return skipReason, err
		}
	}
...
func (fopc *fakeOrphanPodsCleaner) Clean(_ *v1alpha1.HyenaCluster) (map[string]string, error) {
	return nil, nil
}
