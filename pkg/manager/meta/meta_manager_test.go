...
func TestMetaManagerSync(t *testing.T) {
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		// if !test.podRefPvc {
		// 	pod1.Spec = newPodSpec(v1alpha1.ProxyMemberType.String(), pvc1.GetName())
		// }
	...
	tests := []testcase{
		...
		{
			name:             "the volume name of the pod is not store and prophet",
			podHasLabels:     true,
			pvcHasLabels:     true,
			pvcHasVolumeName: true,
			podRefPvc:        false,
		...
		{
			name:             "get cluster ID from Prophet failed",
			podHasLabels:     true,
			pvcHasLabels:     true,
			pvcHasVolumeName: true,
			podRefPvc:        true,
		...
		{
			name:             "get member ID from Prophet failed",
			podHasLabels:     true,
			pvcHasLabels:     true,
			pvcHasVolumeName: true,
			podRefPvc:        true,
		...
		{
			name:             "get store ID from Prophet failed",
			podHasLabels:     true,
			pvcHasLabels:     true,
			pvcHasVolumeName: true,
			podRefPvc:        true,
...
func newPod(hc *v1alpha1.HyenaCluster) *corev1.Pod {
	return &corev1.Pod{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				label.InstanceLabelKey:  hc.GetLabels()[label.InstanceLabelKey],
			},
		},
		Spec: newPodSpec(v1alpha1.ProphetMemberType.String(), "pvc-1"),
	}
}
